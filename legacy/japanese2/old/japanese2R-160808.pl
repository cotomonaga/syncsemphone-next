...
##  @rulename ... [0] は画面上に表示される rule名、[1] は version数を含むファイル名、[2] は single Merge か double Merge かの区別。
  ...
  (
	...
	@plusf = ('', 'partitioning', 'rel-partitioning', 'Bind+partitioning', 'Bind', 'wo', 'ga'); ## [0] が "" なので選択式とか？　でも結局は、$lg ごとに別のサブルーチン
	$plusfnum = @plusf;
...
#####*1 target
	if ($plus[$j] eq "target") {
		push(@{$base[$j][$sy]}, "A=target=x$j-1");
		&show_feature($sy, "", "A=target=x$j-1");
#####*1 partitioning
	} elsif ($plus[$j] eq "partitioning") {  ## wide scope になるやつ
		push(@{$base[$j][$sy]}, "A=partitioning=H,black,x$j-1");
		&show_feature($sy, "", "A=partitioning=H,black,x$j-1");
#####*1 rel-partitioning
	} elsif ($plus[$j] eq "rel-partitioning") {  ## rel clause 内部の partitioning
		push(@{$base[$j][$sy]}, "H=partitioning=H,black,x$j-1");
		&show_feature($sy, "", "H=partitioning=H,black,x$j-1");
#####*1 Bind+partitioning
	} elsif ($plus[$j] eq "Bind+partitioning") {
		push(@{$base[$j][$sy]}, "A=partitioning=H,black,x$j-1");
		&show_feature($sy, "", "A=partitioning=H,black,x$j-1");
		push(@{$base[$j][$sy]}, "H=Bind=0");
		&show_feature($sy, "", "H=Bind=0");
#####*1 Bind
