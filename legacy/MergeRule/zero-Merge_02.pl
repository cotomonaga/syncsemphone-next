...
##  ver.2 ... feature 33 に対応

##@ rule 適用可否チェック
sub zeroMerge_c {
	for($zs=1; $zs<=$semnum[$check]; $zs++){
		@www = split(/,/, $base[$check][$se][$zs]);
		if (($www[1] eq 24) || ($www[1] eq 25) || ($www[1] eq 26) || ($www[1] eq 29) || ($www[1] eq 33)) {
	...
	for($zp=1; $zp<=$prednum[$check]; $zp++){
		...
		if (($www1[1] eq 24) || ($www1[1] eq 25) || ($www1[1] eq 26) || ($www1[1] eq 29) || ($www1[1] eq 33) || ($www2[1] eq 24) || ($www2[1] eq 25) || ($www2[1] eq 26) || ($www2[1] eq 29) || ($www2[1] eq 33)) {
			$rule_c[$zeroMerge][$ch] = $check;
		}
	}
}
