...
sub rare1_c {
	if (($base[$left][$ca] eq "V") && ($base[$right][$ph] eq "-rare-")) {
    for($z=1; $z<=$semnum[$left]; $z++){
      if ($base[$left][$se][$z] =~ m/ga/) {
	      $rule_c[$rare1][0] = $left;
	      $rule_c[$rare1][1] = $right;
	    }
		}
	}
}
...
##		}
		$mo[$se][$z] =~ s/2,33,ga/2,33,ni/g;  ## "★ga"を"★ni"に置き換える
	}
	push(@{$mo[$se]}, "Affectee:2,33,ga");
