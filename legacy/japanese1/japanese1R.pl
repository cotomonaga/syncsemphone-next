...
##  @rulename ... [0] は画面上に表示される rule名、[1] は version数を含むファイル名、[2] は single Merge か double Merge かの区別。
	...
	@rulename = (['','',0],
		...
		['property-da', 'property-da_02', 2],
		['property-Merge', 'property-Merge_01', 2],
		['rel-Merge', 'rel-Merge_02', 2],
		['RH-Merge', 'RH-Merge_02', 2],
		['LH-Merge', 'LH-Merge_02', 2],
  ...
  ($sase1, 
  	...
  	$RHMerge, 
  	$LHMerge,
  	$zeroMerge)=(1..$mergenum); 

	@plusf = ('write'); ## 選択肢を出さない。[0] が 'write' なので記入式、とか？　でも結局は、$lg ごとに別のサブルーチン

sub plus_feature1 {  ## ← lexicon_select
print <<"END";
		<span class="plus"><INPUT type="text" name="plus$i" size="5"></span>
END
}

sub plus_feature2 {  ## ← numeration_arrange
print <<"END";
		<span class="plus"><INPUT type="text" size="2" name="plus$i" value=$plus[$i]></span>
END
}

sub plus_to_numeration {
	push(@{$base[$j][$sy]}, $plus[$j]);
	if (($plus[$j] eq "ga") || ($plus[$j] eq "wo") || ($plus[$j] eq "ni")) {  ##  助詞がついたものは連用句になるから、ということで
		push(@{$base[$j][$sy]}, "2,1,V");
	}
	&show_feature($sy, "", $plus[$j]);
	$word[$no[$j]]=$word[$no[$j]].$plus[$j];
}

sub double_Merge_check {  ## 順番は表示とは無関係。表示は @rulename 順。
	&sase1_c;
	&sase2_c;
	&rare2_c;
	&rare1_c;
