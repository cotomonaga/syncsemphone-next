...
1. 深さ制限付き完全探索（DLS/IDDFS系）を基礎にする。  
   - 第一層の遷移集合が `basenum` を必ず 1 減らす操作のみ（例: RH/LH の二項マージのみ）で構成される場合、初期 `basenum = B0` から深さ上限は `B0-1` で上から抑えられる。  
   - `basenum` 非減少の遷移（単項操作や同一 `basenum` 更新）が混在する場合は、未到達は必ず「深さ `k` 以内で未到達」として返す。  
2. 重複排除は **等価状態の transposition のみ**を使う。  
   - ただし IDDFS と併用する場合、TT（transposition table）は「残り深さ（または探索済み深さ）」を保持し、より深い反復では同一状態を再展開（reopen）できる設計にする。  
   - 深さ情報なしの `visited`（一度見たら以後展開しない）は完全性を損なうため、判定コアでは禁止する。  
...
### 8.3 完全性条件（改訂）
- 深さ上限が根拠付きで定義されていること（`basenum` 単調減少で `B0-1`、または「深さ `k` 以内」限定）。
- IDDFS+TT では深さ情報付き TT を使い、より深い反復で再展開（reopen）できること。
...
1. `independence(a,b)` を RH/LH規則単位で定義する。  
   - 最小安全条件:  
     - 2遷移の read/write 対象ノードID集合が交差しない。  
     - 片方の実行が他方の適用可否（候補化条件）を変えない。  
     - 片方の実行が他方の read 集合を書き換えない（グローバル状態を含む）。  
   - `newnum` については安全側を既定にする。  
     - 既定: `newnum` を更新しうる遷移同士は独立とみなさない。  
     - 緩和する場合は「到達判定と遷移有効性が `newnum` の具体値に依存しない」ことをテストで確認してから行う。  
2. 判定コアでは、削減規則ごとに「到達集合を落とさない根拠」を明記する。  
3. `到達手順なし` を返す条件は、深さ上限内での探索完了時に限定する。  
4. packed は第二層として維持し、第一層（DPOR-aware 完全探索）と直交に併用する。
5. DPOR 独立性の候補は、可能な限り自動検査を併用する。  
   - 候補ペア `(a,b)` に対し `a→b` と `b→a` の両順序を実行し、同値性が崩れたら独立扱いしない（保守側に倒す）。
